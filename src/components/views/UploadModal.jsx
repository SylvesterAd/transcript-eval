...
export default function UploadModal({ onClose, onComplete, initialGroupId, onFilesChange }) {
  ...
  const validateAndAddFiles = useCallback((fileList) => {
    ...
    for (const file of fileList) {
      ...
      if (!exts.includes(ext)) {
        entries.push({ id, name: file.name, file, type: 'video', status: 'error', progress: 0, error: errorMsg, xhr: null, serverId: null, durationSeconds: null })
        continue
      }
      if (file.size > MAX_SIZE) {
        entries.push({ id, name: file.name, file, type: 'video', status: 'error', progress: 0, error: 'File too large (max 50GB)', xhr: null, serverId: null, durationSeconds: null })
        continue
      }

      entries.push({ id, name: file.name, file, type: 'video', status: 'uploading', progress: 0, error: null, xhr: null, serverId: null, durationSeconds: null })
    }

    setFiles(prev => [...prev, ...entries])

    // Probe duration + dimensions in parallel for every entry in this
  ...
  const handleUrlFetch = useCallback(async (urlValue) => {
    if (!urlValue.trim()) return
    try { new URL(urlValue) } catch {
      const id = Date.now() + '-' + Math.random().toString(36).slice(2, 8)
      setFiles(prev => [...prev, {
        id, name: urlValue, file: null, url: urlValue, type: 'video',
    ...
    setFiles(prev => [...prev, { id, name, file: null, url: urlValue, type: 'video', status: 'uploading', progress: 0, error: null, xhr: null, serverId: null }])

    try {
      const gid = await ensureGroup()
      setFiles(prev => prev.map(f => f.id === id ? { ...f, progress: 30 } : f))
      const res = await apiPost('/videos/import-url', { url: urlValue, type: 'video', group_id: gid, title: name })
      setFiles(prev => prev.map(f =>
  ...
  const retryFile = useCallback((fileEntry) => {
    ...
    if (fileEntry.url) handleUrlFetch(fileEntry.url)
                ...
                <div className="flex-1 bg-surface/30 rounded-lg p-4 flex flex-col justify-between border border-border-subtle/5 min-h-[140px]">
                  ...
                  <div className="space-y-2 mt-auto">
                    <input
                      ...
                      onKeyDown={(e) => { if (e.key === 'Enter') handleUrlFetch(videoUrl) }}
                    ...
                    />
                    <button
                      onClick={() => handleUrlFetch(videoUrl)}
