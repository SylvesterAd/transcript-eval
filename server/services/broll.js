...
export async function executeCreateStrategy(prepPipelineId, analysisPipelineId, videoId, groupId) {
  ...
  const pipelineId = `strat-${videoId}-${Date.now()}`
  const pipelineStart = Date.now()
...
export async function executeCreatePlan(prepPipelineId, strategyPipelineId, videoId, groupId) {
  ...
  // Get per-chapter strategy sub-runs ordered by subIndex
  const stratSubRuns = stratRuns
    .filter(r => { const m = JSON.parse(r.metadata_json || '{}'); return m.isSubRun })
    .sort((a, b) => {
      const ma = JSON.parse(a.metadata_json || '{}')
      const mb = JSON.parse(b.metadata_json || '{}')
      return (ma.subIndex || 0) - (mb.subIndex || 0)
  ...
  // Build array of strategy outputs per chapter
  const strategyPerChapter = stratSubRuns.map(r => r.output_text || '')
    ...
    for (let i = 0; i < stages.length; i++) {
      ...
      if (stage.type === 'transcript_question' && stage.per_chapter) {
        ...
        async function processChapter(c) {
          ...
          // Also check for previous per_chapter stages within this pipeline
          for (let pi = i - 1; pi >= 0; pi--) {
            if (stages[pi].type === 'transcript_question' && stages[pi].per_chapter) {
              try { prevChapterOutput = JSON.parse(stageOutputs[pi] || '[]')[c] || '' } catch {}
              break
